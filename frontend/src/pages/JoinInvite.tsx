...
const JoinInvite: React.FC = () => {
  ...
  // Early exit if user has already redeemed invite or account is created
  useEffect(() => {
    if (accountCreated && user?.proId) {
      console.log('✅ User already redeemed invite, redirecting to dashboard...');
      navigate('/app/dashboard');
      return;
    }
  }, [accountCreated, user?.proId, navigate]);
  ...
  const validateInviteToken = async (token: string) => {
    try {
      ...
      if (result.valid) {
        ...
        // Check if this is a claimed invite and redirect immediately
        if (result.error === 'Invite has already been claimed') {
          console.log('⚠️ Invite already claimed, redirecting to dashboard...');
  ...
  useEffect(() => {
    // Prevent multiple triggers - more comprehensive check
    if (hasAttemptedRedemption || submitting || redeemingInvite || accountCreated) {
    ...
    if (user && inviteData && !loading && !redeemingInvite && !accountCreated) {
  ...
  }, [user, inviteData, loading, redeemingInvite, hasAttemptedRedemption, submitting, accountCreated, navigate]);

  // Cleanup effect to handle component unmounting
  useEffect(() => {
    return () => {
      // Cleanup when component unmounts
      console.log('🧹 JoinInvite component unmounting');
    };
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Enhanced guards against duplicate submissions
    if (submitting || redeemingInvite || accountCreated) {
      console.log('⚠️ Form submission blocked:', { submitting, redeemingInvite, accountCreated });
      return;
    }
    ...
    try {
      ...
      await redeemInviteForUser(userCredential.user.uid, formData.email, `${formData.firstName} ${formData.lastName}`);
      
      // Refresh user data to get updated custom claims
      await new Promise(resolve => setTimeout(resolve, 1000)); // Wait for claims to propagate
      if (auth.currentUser) {
        await auth.currentUser.getIdToken(true); // Force refresh token with new claims
      }
      ...
      alert(`🎉 Account created successfully! You've been invited to join the team. Please close this tab and sign in to your new account.`);
      
      // Close the current tab/window and redirect to sign-in page
      try {
        // Try to close the current tab/window
        window.close();
        
        // If window.close() doesn't work (due to browser security), redirect to sign-in
        setTimeout(() => {
          window.location.href = '/auth';
        }, 1000);
      } catch {
        console.log('Could not close window, redirecting to sign-in page...');
        window.location.href = '/auth';
      }
    } catch (error: unknown) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Show success message and close browser
      alert(`🎉 Successfully joined the team! Please close this tab and sign in to your account.`);
  ...
  const redeemInviteForUser = async (uid: string, email: string, displayName: string) => {
    // Enhanced guards against duplicate calls
    if (redeemingInvite || accountCreated) {
      console.log('⚠️ Already redeeming invite or account created, skipping...', { redeemingInvite, accountCreated });
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (errorData.error === 'Invite has already been claimed') {
          console.log('⚠️ Invite already claimed, marking account as created and redirecting...');
          setAccountCreated(true);
          setError('This invite has already been used. Redirecting to dashboard...');
      ...
      // Show success message and close browser
      alert(`🎉 You have been successfully connected to the team! Please close this tab and sign in to your account.`);
      ...
      <div className="max-w-md w-full bg-white dark:bg-gray-800 rounded-lg shadow-lg p-8">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">
            {inviteData.proUserName && inviteData.teamName 
              ? `You're invited by ${inviteData.proUserName} to join ${inviteData.teamName}`
              : 'Join the Team!'
            }
          </h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            {submitting ? 'Creating Account...' : `Create Account & Join as ${inviteData.role}`}
          </button>
        </form>
